...
Component({
  ...
  methods: {
    ...
    bindRectChange() {
      ...
      this.createIntersectionObserver({
        ...
        if (res) {
          ...
          var top = Math.min(bound.top, glob.data.headerHeight && glob.data.isRefresh ? bound.top - glob.data.headerHeight : bound.top)
          ...
          var contentBottom = Math.max(bound.bottom - top, contentHeight)

          glob.setData({
            contentBottom: contentBottom,
            contentHeight: contentHeight,
            contentTop: bound.top
          })
          _invalidate(glob)
        }
      })
    ...
    onTouchEnd(event) {
      ...
      if (offset > 0 && this.data.scroll > 0) {
        ...
        if (!this.data.enableRefresh) {
          this._closeRefresh()
        } else if (this.data.scroll > (this.data.headerHeight >> 1)) {
          this._openRefresh()
        } else {
          this._closeRefresh()
        }
  ...
  observers: {
    ...
    contentBottom(val) {
      ...
      // console.log('contentBottom---->' + val)
    },
    contentHeight(val) {
      // console.log('contentHeight---->' + val)
    },
    contentTop(val) {
      // console.log('contentTop---->' + val)
    }
  },
